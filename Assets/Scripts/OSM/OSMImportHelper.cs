...
public class OSMImportHelper
{
    ...
    public OSMImportHelper(OSMImporterProxy importer, string area, string file)
    {
        ...
        case Area.ProvidenceRI :
        {
            string allNodesFileName = "Resources/OSM/";
            allNodesFileName += "RhodeIsland";
            allNodesFileName += ".osm.pbf";
            var allNodesInput = File.OpenRead(allNodesFileName);
            if (allNodesInput == null)
            {
    ...
    if (
        ...
        tags.Contains("water", "river")
    )

                    {
                        importer.naturalFeatures.Add(new Tuple<OsmGeo, NaturalFeature.Type>(geo, NaturalFeature.Type.River));
                        AddGeoReference(geo);
                    } else 
    if (
    ...
    if (
        tags.Contains("waterway", "riverbank") &&  ! 
        tags.ContainsKey("tunnel")
    )
    ...
    if (
        ...
        case Area.ProvidenceRI :
        {
            foreach (var geo in sourceStream)
            {
                ...
                if (geo.Type == OsmGeoType.Relation)
                {
                    string boundaryName = "Providence";
                    if (tags.Contains("name", boundaryName))
                    {
                        importer.boundary = geo as Relation;
                        AddGeoReference(geo);
    ...
    if (
        ...
        case Area.JerseyCity :
        {
            foreach (var geo in sourceStream)
            {
                ...
                if (geo.Type == OsmGeoType.Relation)
                {
                    string boundaryName = "Jersey City";
                    if (tags.Contains("name", boundaryName))
                    {
                        importer.boundary = geo as Relation;
                        AddGeoReference(geo);
